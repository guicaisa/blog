...
### 解法
  1. 二分查找
  * 由于numbers数组是升序排列的，所以可以使用二份查找的方式来处理。在遍历数组的过程中，选定一个数字，然后在其之后的范围中进行二分查找，找到与其相加等于target的值即可获得结果，如果相加结果大于target，表示需要更小的值，则下次二分查找定位到左边区间，反之定位到右边区间
    ![](./p1.gif)
    ```C++
  ...
  2. 双指针
  * 左右指针相向遍历，相加之后与target判断大小，如果相等就直接返回答案，由于数组是升序排列的，根据相加的值与target大小的判断，来定义下次的遍历方向，比target小，需要更大的和，所以左边的指针右移，反之亦然
    ![](./p2.gif)
    ```C++
